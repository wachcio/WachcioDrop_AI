...
| `wachciodrop/group/{1-10}/command` | `{"on":true,"duration":300}` | Włącz grupę z timerem |
| `wachciodrop/group/{1-10}/command` | `{"on":false}` | Wyłącz wszystkie sekcje grupy |
| `wachciodrop/schedule/set` | JSON array (format jak GET /api/schedule) | Ustaw harmonogram |

### Stany (subscribe ← ESP32)

| Topik | Wartość | Opis |
|-------|---------|------|
| `wachciodrop/section/{1-8}/state` | `ON` / `OFF` | Stan sekcji |
| `wachciodrop/section/master/state` | `ON` / `OFF` | Stan zaworu głównego |
| `wachciodrop/status` | JSON | IP, RSSI, uptime, irrigation_today, online |

### Przykłady mosquitto_pub
...
# Wyłącz wszystko
mosquitto_pub -h 192.168.20.10 \
  -t "wachciodrop/section/all/command" \
  -m "OFF"

# Subskrybuj stany wszystkich sekcji
mosquitto_sub -h 192.168.20.10 \
  -t "wachciodrop/section/+/state"
```
